...
void VK_TextureMode(const char *string) {
    ...
    for ( i = 0 ; i < tr.numImages ; i++ ) {
        ...
        if (glt->mipmap) {
			...
			VK_SetSamplerPosition(&vk_d.imageDescriptor, 0, VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_CLOSEST_HIT_BIT_NV, image->sampler, image->view, glt->index);
...
static struct Image_Upload_Data generate_image_upload_data(const byte* data, int width, int height, qboolean mipmap, qboolean picmip) {
	//
...
image_t *R_CreateImage(const char *name, const byte *pic, int width, int height,
	...
	if (glConfig.driverType == OPENGL) {
		...
		VK_SetSamplerPosition(&vk_d.imageDescriptor, 0, VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_CLOSEST_HIT_BIT_NV, vk_d.images[image->index].sampler, vk_d.images[image->index].view, image->index);
		VK_SetUpdateSize(&vk_d.imageDescriptor, 0, VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_CLOSEST_HIT_BIT_NV, image->index+1);
		VK_UpdateDescriptorSet(&vk_d.imageDescriptor);
	}
...
static void jpeg_write_func(void* context, void* data, int size) {
	struct Write_Context* ctx = (struct Write_Context*)(context);
...
void SaveJPG(char * filename, int quality, int image_width, int image_height, unsigned char *image_buffer) {
	...
	byte* buffer = (byte*)ri.Hunk_AllocateTempMemory(image_width * image_height * 4);
	int row_bytes = image_width * 4;
	for (int i = 0; i < image_height; i++) {
		memcpy(&buffer[i * row_bytes], &image_buffer[(image_height - 1 - i) * row_bytes], row_bytes);
	}

	int capacity1 = 1024 * 1024;
	struct Write_Context context;
	context.size = 0;
	context.capacity = capacity1;
	context.buffer = (byte*)ri.Malloc(capacity1);
