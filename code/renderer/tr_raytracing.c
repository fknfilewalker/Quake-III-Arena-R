...
static void RB_UpdateRayTraceAS(drawSurf_t* drawSurfs, int numDrawSurfs) {
	...
	for (i = 0, drawSurf = drawSurfs; i < numDrawSurfs; i++, drawSurf++) {
		...
		else if (*drawSurf->surface == SF_POLY) idxBottomAS = ((srfPoly_t*)drawSurf->surface)->idxBottomAS;
		...
		//int entityNum = (drawSurf->sort >> QSORT_ENTITYNUM_SHIFT) & 1023;
		if (idxBottomAS == 4800) {
			int x= 2;
		}

		if (idxBottomAS >= 0 && idxBottomAS < vk_d.bottomASCount) {
			qboolean cTex = (tess.shader->stages[0]->bundle[0].tcGen != TCGEN_BAD)&& tess.shader->stages[0]->bundle[0].numTexMods > 0;
			qboolean defor = tess.shader->numDeforms > 0;
			if (defor | cTex) {
				if (defor) RB_DeformTessGeometry();
				...
				for (j = 0; j < tess.numVertexes; j++) {
					...
					VK_UploadBufferDataOffset(&vk_d.geometry.xyz, vk_d.bottomASList[idxBottomAS].geometries.geometry.triangles.vertexOffset + (j * 12 * sizeof(float)), 12 * sizeof(float), (void*)&p);
				}
				if (defor) VK_UpdateBottomSingle(&vk_d.bottomASList[idxBottomAS]);
			}
			if (tess.shader->numDeforms) {
				...
				//VK_UpdateBottomSingle(&vk_d.bottomASList[idxBottomAS]);
			}
			//if (tess.shader->stages[0]->bundle[0].numImageAnimations > 1) {
			//	int		index;
			//	index = (int)(tess.shaderTime * tess.shader->stages[0]->bundle[0].imageAnimationSpeed * FUNCTABLE_SIZE);
			//	index >>= FUNCTABLE_SIZE2;

			//	if (index < 0) {
			//		index = 0;	// may happen with shader time offsets
			//	}
			//	index %= tess.shader->stages[0]->bundle[0].numImageAnimations;
			//	vk_d.bottomASList[idxBottomAS].data.texIdx = index;
			//	VK_UploadBufferDataOffset(&vk_d.instanceDataBuffer, idxBottomAS * sizeof(ASInstanceData), sizeof(ASInstanceData), (void*)&vk_d.bottomASList[idxBottomAS].data);
			...
			VK_UploadBufferDataOffset(&vk_d.instanceBuffer, idxBottomAS * sizeof(VkGeometryInstanceNV), sizeof(VkGeometryInstanceNV), (void*)&geometryInstance);
		}
	}

	// update as
	VK_UpdateTop(&vk_d.topAS);
