...
void R_AddMD3Surfaces( trRefEntity_t *ent ) {
	...
	for ( i = 0 ; i < header->numSurfaces ; i++ ) {
		...
		if ( !personalModel
			...
			R_AddDrawSurf( (void *)surface, tr.shadowShader, 0, qfalse, tr.currentModel ->bAS[lod][i]/*vk_d.bottomASCount + ent->e.hModel*/);
		}
		...
		if ( r_shadows->integer == 3
			...
			&& shader->sort == SS_OPAQUE ) {
			R_AddDrawSurf( (void *)surface, tr.projectionShadowShader, 0, qfalse, tr.currentModel->bAS[lod][i]);
		}
