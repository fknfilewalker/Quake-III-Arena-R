...
qboolean RB_IsTransparent(shader_t* shader) {
	...
	if (strstr(shader->name, "health/mega1")) return qfalse;
	
	if (strstr(shader->name, "armor/energy_yel1")) return qtrue;
	if (strstr(shader->name, "f_plasma")) return qtrue;
	if (strstr(shader->name, "slashskate")) return qtrue;
	if (strstr(shader->name, "railCore")) return qtrue;
	if (strstr(shader->name, "f_machine")) return qtrue;
	if (strstr(shader->name, "f_shotgun")) return qtrue;
	if (strstr(shader->name, "f_railgun")) return qtrue;
	if (strstr(shader->name, "shotgun_laser")) return qtrue;
	if (strstr(shader->name, "plasma_glo")) return qfalse;
	if (strstr(shader->name, "viewBloodBlend")) return qtrue;
	if (strstr(shader->name, "railgun2")) return qtrue;
	if (strstr(shader->name, "f_rocket")) return qtrue;
	if (strstr(shader->name, "console/sphere2")) return qfalse;
	if (strstr(shader->name, "portal") && !strstr(shader->name, "portal_2")) return qtrue;
	
	if ((shader->contentFlags & CONTENTS_TRANSLUCENT) == CONTENTS_TRANSLUCENT && shader->sort == SS_OPAQUE) return qfalse;
	if ((shader->contentFlags & CONTENTS_TRANSLUCENT) == CONTENTS_TRANSLUCENT || shader->sort > SS_OPAQUE) return qtrue;
	return qfalse;
}
